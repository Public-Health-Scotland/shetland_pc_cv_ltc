...
These indicators have been produced by LIST (the Local Intelligence Support Team), in collaboration with NHS Shetland. Patient level data has been extracted from SHIP, it was then anonymised before being sent securely to PHS.
...
For identifying patients with a long-term condition, we look for a relevant 'Record of Diagnosis' event and start counting them from that month onwards. We will stop counting a patient if they die, or if we have a record indicating that they have left Shetland. We carry out this process Shetland-wide and assign patients to a practice based on their most recent data. This method is quite reliable, as it takes into account any 'Joined practice' events, and it remains effective even if some information hasn't been coded or updated.

Our methodology differs from SHIP in at least these known ways:

-   We start counting a patient immediately from diagnosis, whereas SHIP implements a 15-month lag from the first diagnosis when assessing if a review is due (this affects the invite and attendance indicators.

-   ~~SHIP 'allows' some LTCs to be 'resolved', we count patients in perpetuity (unless we know they have died or left Shetland).~~
...
event_type_colours <- c("#12436D", "#28A197", "#801650") |>
  setNames(unique(pull(quarterly_event_type, DerivedEventType)))
...
pharm_9_plotly <- shared_monthly_summary$data() |>
  ...
  group_map(
    \(practice_data, practice_id) {
      ...
      first_practice_id <- unique(pull(
        shared_monthly_summary$data(),
        PracticeID
        ))[1]
...
pharma1_90_dt <- make_dt(
  shared_data = shared_monthly_summary,
  var = "prop_review_90",
  name = "Proportion Reviewed (90 days)",
  percentage = TRUE
)
```
...
pharma1_180_plotly <- plot_ly(
  ...
  mode = "lines+markers",
  hoverinfo = "text",
  text = ~ paste("Practice:", PracticeID, "<br>Month:", census_date, "<br>Proportion Reviewed (180 days):", percent(prop_review_180, accuracy = 0.1))
) |>
  layout(
...
pharma1_180_dt <- make_dt(
  shared_data = shared_monthly_summary,
  var = "prop_review_180",
  name = "Proportion Reviewed (180 days)",
  percentage = TRUE
)
```
