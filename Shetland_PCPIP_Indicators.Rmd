...
all_data <- monthly_summary |>
  ...
  mutate(
    ...
    show_legend_flag = !duplicated(DerivedEventType)
  )
...
This looks at the prevalence of patients identified as having Cardiovascular Disease per practice. This is relevant as it is these patients that are used as the denominator in the following indicators.
...
cv_prev_plotly <- plot_ly(
  ...
  mode = "lines+markers",
  hoverinfo = "text",
  text = ~ paste("Practice:", PracticeID, "<br>Month:", census_date, "<br>CV Prevelance:", percent(list_prev, accuracy = 0.1))
) |>
  layout(
    title = str_wrap("Monthly CV Prevelance percentage by GP Practice", 25),
    xaxis = list(title = "Month"),
    yaxis = list(
      title = "CV Prevelance percentage",
      tickformat = ".1%"
    ),
    showlegend = TRUE,
    legend = list(
...
event_type_colours <- phs_colours(c("phs-purple", "phs-magenta", "phs-blue")) |>
...
file_path <- "/conf/LIST_analytics/Shetland/Primary Care/LTC/data/outputs/Indicators.xlsx"
